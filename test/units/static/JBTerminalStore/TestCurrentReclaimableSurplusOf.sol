...
contract TestCurrentReclaimableSurplusOf_Local is JBTerminalStoreSetup {
    ...
    function test_GivenCurrentSurplusEqZero() external {
        ...
        mockExpect(address(_controller), abi.encodeCall(IJBController.FUND_ACCESS_LIMITS, ()), abi.encode(_accessLimits));
        ...
        bytes memory _payoutLimitsCall = abi.encodeCall(IJBFundAccessLimits.payoutLimitsOf, (_projectId, block.timestamp, address(_terminal), address(_token)));
        ...
        uint256 reclaimable = _store.currentReclaimableSurplusOf(address(_terminal), _projectId, _contexts, 18, _currency, _tokenCount, false);
    ...
    function test_GivenRedemptionRateEqMAX_REDEMPTION_RATE() external whenUseTotalSurplusEqFalse whenProjectHasBalance {
        // it will return the linear proportion
    }

    function test_GivenRedemptionRateDneqMAX_REDEMPTION_RATE() external whenUseTotalSurplusEqFalse whenProjectHasBalance {
