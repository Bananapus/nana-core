...
interface IJBRulesetDataHook is IERC165 {
    /// @notice The data calculated before a payment is processed. This is provided to the terminal's `pay(...)`
    /// transaction.
    /// @param context The context passed to this data hook by the `pay(...)` function as a `JBBeforePayContext` struct.
    ...
    function beforePay(JBBeforePayContext calldata context)
        external
        view
        returns (uint256 weight, JBPayHookSpecification[] memory hookSpecifications);

    /// @notice The data calculated before a redemption is processed. This is provided to the terminal's
    /// `redeemTokensOf(...)` transaction.
    /// @param context The context passed to this data hook by the `redeemTokensOf(...)` function as a
    /// `JBBeforeRedeemContext` struct.
    ...
    function beforeRedeem(JBBeforeRedeemContext calldata context)
        external
        view
